...
int main(int argc, char **argv)
{
    ...
    desc.add_options()
        ...
        ("count",    boost::program_options::value<uint32_t>(), "number of atomics to issue")
        ("inflight", boost::program_options::value<uint32_t>(), "number of outstanding atomics allowed")
    ;
    ...
    cout <<"Atomics Benchmark (" << count << ", " << inflight << ")" << endl;
    ...
    opbox::RegisterOp<OpBenchmarkCompareSwap>();
    opbox::RegisterOp<OpBenchmarkFetchAdd>();
    ...
    if(G.mpi_rank==0){
        OpBenchmarkCompareSwap *op = new OpBenchmarkCompareSwap(G.peers[1], count, inflight);
        future<uint32_t> fut = op->GetFuture();
        opbox::LaunchOp(op);
        uint32_t res=fut.get();
        cout<<"CompareSwap result is '"<<res<<"'\n";
    }
    if(G.mpi_rank==0){
        OpBenchmarkFetchAdd *op = new OpBenchmarkFetchAdd(G.peers[1], count, inflight);
        future<uint32_t> fut = op->GetFuture();
        opbox::LaunchOp(op);
        uint32_t res=fut.get();
        cout<<"FetchAdd result is '"<<res<<"'\n";
    }
