...
const unsigned int OpBenchmarkCompareSwap::op_id = const_hash("OpBenchmarkCompareSwap");
const string OpBenchmarkCompareSwap::op_name = "OpBenchmarkCompareSwap";


OpBenchmarkCompareSwap::OpBenchmarkCompareSwap(opbox::net::peer_ptr_t dst, uint32_t count, uint32_t max_inflight)
        : faodel::LoggingInterface("Opbox", "OpBenchmarkCompareSwap"),
          state(State::start),
...
OpBenchmarkCompareSwap::OpBenchmarkCompareSwap(op_create_as_target_t t)
        : faodel::LoggingInterface("Opbox", "OpBenchmarkCompareSwap"),
          state(State::start),
...
OpBenchmarkCompareSwap::~OpBenchmarkCompareSwap() {
}

future <uint32_t> OpBenchmarkCompareSwap::GetFuture() {
 ...
 * ping message.
 */
void OpBenchmarkCompareSwap::createOutgoingMessage(faodel::nodeid_t dst,
  ...
  msg->op_id         = OpBenchmarkCompareSwap::op_id;
 ...
 */
void OpBenchmarkCompareSwap::createAckMessage(faodel::nodeid_t dst,
                                              const mailbox_t &src_mailbox,
                                              const mailbox_t &dst_mailbox) {
...
WaitingType OpBenchmarkCompareSwap::UpdateOrigin(OpArgs *args) {
  switch(state) {
    case State::start:
    ...
    case State::snd_wait_for_ack:
      ...
      assert(args->type == UpdateType::incoming_message &&
             "Sender in snd_wait_for_ack, but event not an incoming message");
...
WaitingType OpBenchmarkCompareSwap::UpdateTarget(OpArgs *args) {
  stringstream ss;
  ...
  switch(state) {
    ...
    case State::start: {
      ...
      while(started<max_inflight) {
        ...
        opbox::net::Atomic(peer, opbox::net::AtomicOp::CompareSwap, atomics_ldo,
                           atomics_ldo.GetHeaderSize() + atomics_ldo.GetMetaSize(), &nbr,
                           0, 8, started*10, (started + 1)*10, AllEventsCallback(this));
        started++;
        inflight++;
        ss.str(std::string());
...
string OpBenchmarkCompareSwap::GetStateName() const {
  switch(state){
