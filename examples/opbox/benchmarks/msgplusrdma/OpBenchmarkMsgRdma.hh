...
#ifndef OPBENCHMARKMSGRDMA_HH
#define OPBENCHMARKMSGRDMA_HH
...
public:
    OpBenchmarkMsgRdma(opbox::net::peer_ptr_t dst, uint32_t size, uint32_t count, uint32_t max_inflight);
    OpBenchmarkMsgRdma(op_create_as_target_t t);
    ~OpBenchmarkMsgRdma();
...
private:
    ...
    faodel::nodeid_t src_nodeid;   //!< The origin's id (only used by the op)
    ...
    void updateOutgoingMessage(const mailbox_t &dst_mailbox);
