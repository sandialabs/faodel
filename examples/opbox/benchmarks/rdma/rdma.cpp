...
int main(int argc, char **argv) {
  ...
  cout << "RDMA Benchmark (" << count << ", " << inflight << ", " << length << ")" << endl;
  ...
  opbox::RegisterOp<OpBenchmarkGet>();
  opbox::RegisterOp<OpBenchmarkPut>();
  ...
  if(G.mpi_rank == 0) {
    OpBenchmarkGet *op = new OpBenchmarkGet(G.peers[1], length, count, inflight);
  ...
  if(G.mpi_rank == 0) {
    OpBenchmarkPut *op = new OpBenchmarkPut(G.peers[1], length, count, inflight);
    future <uint32_t> fut = op->GetFuture();
    opbox::LaunchOp(op);
    uint32_t res = fut.get();
    cout << "Second result is '" << res << "'\n";
