    ...
    {   
      private:
        ...
        NNTI_result_t recv_callback_func(NNTI_event_t *event, void *context)
        {
          ...
          DataObject memory(nnti_attrs.mtu-nnti_attrs.max_eager_size, nnti_attrs.max_eager_size, 
...
TEST_F(LunasaSendTest, basic)
{
    ...
    if( mpi_rank < partner_rank ) {
        ...
        /* ALLOCATE space for message header, length/seed, and pseudo-random data */
        uint32_t header_length = nnti_attrs.mtu-nnti_attrs.max_eager_size;
        char *user_memory = (char *)malloc(header_length+4+payload_length);

        DataObject memory(user_memory, 0, header_length+payload_length+4, cleanup);

        /* SKIP past header */
        char *payload = user_memory + header_length;

        /* READ the payload length and the PRNG seed */
        ...
        *payload++ = seed & 0xff;
        ...
        // Our user LDO is comprised of two memory regions: (i) headers; and (ii) user memory.
        // For this test, we're only interested in the user memory, so we skip the headers.
        rdma_segments.pop();
        DataObject::rdma_segment_desc rdma_segment = rdma_segments.front();
        rdma_buffer = (NNTI_buffer_t)rdma_segment.net_buffer_handle;
        rdma_offset = rdma_segment.net_buffer_offset;
