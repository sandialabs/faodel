...
TEST_F(LunasaBasic, simpleAlloc){
  ...
  EXPECT_EQ(0, memcmp(obj.GetHeaderPtr(), read_obj.GetHeaderPtr(), 
            obj.GetHeaderSize() + obj.GetMetaSize() + obj.GetDataSize()));
...
TEST_F(LunasaBasic, multipleAllocs){
  ...
  //cerr <<"Total Allocatedd: "<<lunasa_instance.TotalAllocated()<<endl;
