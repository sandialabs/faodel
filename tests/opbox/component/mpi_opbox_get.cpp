...
void calc_crc(char *base, uint32_t length, uint32_t seed) {
    ...
    *(uint32_t*)base = crc;
    ...
    fprintf(stderr, "sender: length=%u seed=0x%x  base[0]=0x%08x  base[%d]=0x%x  crc=0x%08x\n",
        length, seed, *(uint32_t*)base, length-4, *(uint32_t*)&base[length-4], crc);
}
...
void verify_crc(char *base, uint32_t length) {
    ...
    fprintf(stderr, "receiver: length=%u seed=0x%x  payload[0]=0x%08x  payload[%d]=0x%x  crc=0x%08x\n",
        length, seed, *(uint32_t*)base, length-4, *(uint32_t*)&base[length-4], crc);
...
TEST_F(OpboxGetTest, start1) {
    ...
    if(mpi_rank == root_rank){
        ...
        calc_crc((char*)get_target.GetDataPtr(), get_target.GetDataSize(), 1);
