...
int main(int argc, char *argv[])
{
    ...
    setenv("NNTI_LOG_FILENAME", "IBQueueSendTest2.log", 1);

    rc = NNTI_init(transport_id, req_url, &transport);
    assert(rc==NNTI_OK);
    log_debug("IBQueueSendTest2", "Init ran");
    ...
    log_debug("IBQueueSendTest2", "Is initialized");
    rc = NNTI_get_url(transport, my_url, NNTI_URL_LEN);
    //Dies here?
    assert(rc==NNTI_OK);

    log_debug("IBQueueSendTest2", "my_url=%s", my_url);
    ...
    if (i_am_server) {
        ...
        if (rc != NNTI_OK) {
            log_error("IBQueueSendTest2", "recv_target_hdl() failed: %d", rc);
        }

        rc = send_target_hdl(transport, src_hdl, src_base, src_size, my_q_hdl, peer_hdl, eq);
        if (rc != NNTI_OK) {
            log_error("IBQueueSendTest2", "send_target_hdl() failed: %d", rc);
        }
