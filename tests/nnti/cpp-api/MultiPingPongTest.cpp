...
typedef std::map<NNTI_peer_t, struct buffer_properties *> buffer_properties_map_t;
typedef std::map<NNTI_peer_t, struct counts *> counts_map_t;
...
public:
    ...
    buffer_properties_map_t              send_target_map_;
    counts_map_t                         counts_map;
        ...
        switch (event->type) {
            case NNTI_EVENT_SEND:
                ...
                if (c->send_count_ < c->volley_threshold_) {
                    ...
                    log_debug("pingpong_callback", "send_count_(%lu) reached threshold", c->send_count_);
            ...
            case NNTI_EVENT_RECV:
                ...
                if (c->recv_count_ < c->volley_threshold_) {
                    ...
                    log_debug("pingpong_callback", "recv_count_(%lu) reached threshold", c->recv_count_);
                    // we're done here.  return NNTI_ECANCEL to push an event on the EQ.
                    return NNTI_ECANCELED;
                }
                break;
...
class NntiMultiPingPongTest : public testing::Test {
protected:
...
TEST_F(NntiMultiPingPongTest, start1) {
    NNTI_result_t rc;
    std::list<NNTI_peer_t> peers;
    ...
    NNTI_event_queue_t  send_eq;
    NNTI_event_queue_t  pingpong_eq;
    NNTI_event_t        event;

    struct buffer_properties send_buf;
    struct buffer_properties recv_buf;
    struct buffer_properties my_pingpong_buf;

    if (i_am_server) {
        volley_count *= num_clients;
        volley_count += (num_clients-1);
    }

    log_debug("MultiPingPongTest", "num_clients=%u  volley_count=%u", num_clients, volley_count);

    rc = t->eq_create(128, NNTI_EQF_UNEXPECTED, &unexpected_eq);
    rc = t->eq_create(128, NNTI_EQF_UNSET, &send_eq);
    ...
    if (i_am_server) {
        ...
        for (int i=0;i<num_clients;i++) {
            ...
            if (rc != NNTI_OK) {
                log_error("MultiPingPongTest", "recv_target_hdl() failed: %d", rc);
            }

            peers.push_back(peer_hdl);

            ppc->send_target_map_[peer_hdl] = peer_pingpong_buf;
            ppc->counts_map[peer_hdl] = new struct counts;

            rc = send_target_hdl(t, send_buf.hdl, send_buf.base, send_buf.size, my_pingpong_buf.hdl, peer_hdl, send_eq);
            if (rc != NNTI_OK) {
                log_error("MultiPingPongTest", "send_target_hdl() failed: %d", rc);
            }
        }
        ...
        for (auto p : peers) {
            ...
            rc = send_target_hdl(t, send_buf.hdl, send_buf.base, send_buf.size, my_pingpong_buf.hdl, p, send_eq);
            if (rc != NNTI_OK) {
                log_error("MultiPingPongTest", "send_target_hdl() failed: %d", rc);
            }
        }
...
//        nnti::util::sleep(1000);
        ...
        rc = recv_target_hdl(t, recv_buf.hdl, recv_buf.base, &peer_pingpong_buf->hdl, &recv_peer, unexpected_eq);
        if (rc != NNTI_OK) {
            log_error("MultiPingPongTest", "recv_target_hdl() failed: %d", rc);
        }
