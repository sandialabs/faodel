...
class NntiQueueSendTest2 : public testing::Test {
protected:
...
TEST_F(NntiQueueSendTest2, start1) {
    NNTI_result_t rc;
    ...
    if (i_am_server) {
        ...
        if (rc != NNTI_OK) {
            log_error("QueueSendTest2", "recv_target_hdl() failed: %d", rc);
        }

        rc = send_target_hdl(t, src_hdl, src_base, src_size, my_q_hdl, peer_hdl, eq);
        if (rc != NNTI_OK) {
            log_error("QueueSendTest2", "send_target_hdl() failed: %d", rc);
        }
...
//        nnti::util::sleep(1000);
        ...
        log_debug("QueueSendTest2", "populate and send took %lu nanoseconds", std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
