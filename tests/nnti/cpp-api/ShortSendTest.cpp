...
class NntiShortSendTest : public testing::Test {
protected:
...
TEST_F(NntiShortSendTest, start1) {
    NNTI_result_t rc;
    ...
    if (i_am_server) {
        ...
        for (int j=0;j<100;j++) {
            ...
            for (int i=0;i<10;i++) {
                EXPECT_TRUE(verify_buffer((char*)event.start, i*(buf_size/10), event.length));
            }
        }
        ...
        for (int j=0;j<100;j++) {
            for (int i=0;i<10;i++) {
                rc = send_data(t, i, buf_hdl, target_hdl, peer_hdl, eq);
            }
        }
        ...
        rc = t->eq_create(128, NNTI_EQF_UNEXPECTED, &eq);
        rc = t->alloc(3200, NNTI_BF_LOCAL_READ, eq, obj_cb, nullptr, &buf_base, &buf_hdl);
