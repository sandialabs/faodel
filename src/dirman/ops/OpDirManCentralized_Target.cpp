...
WaitingType OpDirManCentralized::UpdateTarget(OpArgs *args){
  ...
  case State::start:
    {
      ...
      if(req_type == RequestType::HostNewDir){
        ...
        switch(req_type){
        case RequestType::GetInfo:  dirman::GetLocalDirectoryInfo(url, &result_dir_info); break;
        case RequestType::JoinDir:  dirman::JoinDirWithName(url, "",   &result_dir_info); break;
        case RequestType::LeaveDir: dirman::LeaveDir(url,              &result_dir_info); break;
        default: //Unknown?
  ...
  default:
    KTODO("Unhandled state in OpDirManCentralized receiver Update");
  }
